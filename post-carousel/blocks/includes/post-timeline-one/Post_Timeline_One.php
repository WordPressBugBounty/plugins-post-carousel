<?php
/**
 * Post timeline block class for Smart Post Show Blocks.
 *
 ...
 * Post_Timeline_One
 */
class Post_Timeline_One extends Block_Base {
	/**
	...
	protected function set_block_properties() {
		$this->block_name     = 'post-timeline-one';
	...
	/**
	 * Render the Post Timeline One block.
	 *
	...
	public function render_block( $attributes, $content = '', $blocks = array() ) {
		...
		// $is_mobile_class = wp_is_mobile() ? 'timeline-one-layout-five' : $layout;
		...
		<div id="<?php echo esc_attr( $unique_id ); ?>" class="sp-smart-post-wrapper sp-smart-<?php echo esc_attr( $block_name ); ?> <?php echo esc_attr( $sp_user_additional_class ); ?> <?php echo esc_attr( $pagination_class ); ?>"  data-query='<?php echo esc_attr( $query_attr ); ?>' data-blockid="<?php echo esc_attr( $sp_block_id ); ?>" data-pageid="<?php echo esc_attr( $attributes['page_id'] ); ?>" data-location="<?php echo esc_attr( $block_location ); ?>">
			...
			<div class="post-timeline-one smart-post-timeline-one sp-smart-post-show-scroll-wrapper img-position-<?php echo esc_attr( $image_position ); ?>  <?php echo esc_attr( $orientation . ' ' . $layout ); ?> sp-cat-position-<?php echo esc_attr( $category_position ); ?>" id="<?php echo esc_attr( $unique_id ); ?>">
				<div class="sp-smart-post-timeline-container <?php echo esc_attr( $layout ); ?> sp-smart-post-items">
					<?php
					Template::post_timeline_one( $attributes, $post_query );
					?>
				</div>
			</div>
			<?php
